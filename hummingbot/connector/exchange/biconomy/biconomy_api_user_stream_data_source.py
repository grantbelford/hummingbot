...
class BiconomyAPIUserStreamDataSource(UserStreamTrackerDataSource):
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        """
        self._manage_listen_key_task = safe_ensure_future(
            self._manage_listen_key_task_loop())
        ...
        try:
            data = await rest_assistant.execute_request(
                url=web_utils.public_rest_url(
                    path_url=CONSTANTS.BICONOMY_USER_STREAM_PATH_URL, domain=self._domain),
                method=RESTMethod.POST,
                throttler_limit_id=CONSTANTS.BICONOMY_USER_STREAM_PATH_URL,
        ...
        try:
            while True:
                ...
                if self._current_listen_key is None:
                    ...
                    self.logger().info(
                        f"Successfully obtained listen key {self._current_listen_key}")
                ...
                if now - self._last_listen_key_ping_ts >= self.LISTEN_KEY_KEEP_ALIVE_INTERVAL:
                    ...
                    if not success:
                        ...
                        self.logger().info(
                            f"Refreshed listen key {self._current_listen_key}.")
                        self._last_listen_key_ping_ts = int(time.time())
                else:
